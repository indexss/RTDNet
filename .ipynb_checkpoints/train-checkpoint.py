...
from eval import eyediap_mid_test
...
if __name__ == '__main__':
    ...
    train_dataset_paths = [
        ('eve_train',
         EVESequences_train,
        #  '/home/linlishi/dataset/eve_dataset',
    ...
    # scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizers, T_max=50000, eta_min=0, last_epoch=-1)
    ...
    for epoch in range(num_epochs):
        ...
        for i, data in enumerate(dataloader):
            ...
            if i == num_batches // 2:
                ...
                path = "./saving/eveframe123_epoch_" + str(epoch) +"_batch_" + str(i)+"_time_"+utc_time.strftime('%Y%m%d_%H%M%S')+".pth"
                torch.save(model.state_dict(), path)

                # gaze_diff = eyediap_mid_test(config, device, run=None, model=model)
                model.train()
            ...
            wandb.log({"full_loss": output_dict['full_loss'].item(),
                    ...
                    #    'loss_l1_right_pupil_size': output_dict['loss_l1_right_pupil_size'].item(),
                        'loss_ada_face': output_dict['face_ada_loss'].item(),
                    #    'loss_ada_right': output_dict['right_ada_loss'].item(),
                       'w_ada_face': output_dict['face_weighted_ada_loss'].item(),
                        # 'w_ada_right': output_dict['right_weighted_ada_loss'].item(),
                       'loss_ang_face_g_initial': output_dict['loss_ang_face_g_initial'].item(),
                    #    'loss_ang_right_g_initial': output_dict['loss_ang_right_g_initial'].item(),
                       'learning_rate': optimizers.state_dict()["param_groups"][0]["lr"],
                       'loss_euc_face_PoG_cm_initial': output_dict['euc_loss_face_PoG_cm_initial'].item(),
                    #    'loss_euc_right_PoG_cm_initial': output_dict['euc_loss_right_PoG_cm_initial'].item(),
                       'loss_euc_face_PoG_px_initial': output_dict['euc_loss_face_PoG_px_initial'].item(),
            ...
            time_mid = time.time()
            time_diff = time_mid - time_begin # in seconds
            # 目前batch数
            batch_now = epoch * num_batches + i+1
            # 剩余batch数
            batch_left = num_epochs * num_batches - batch_now
            # 剩余时间
            time_left = time_diff / batch_now * batch_left
            # in hours
            time_left = time_left / 3600

            print(f"Epoch [{epoch + 1}/{num_epochs}], Batch [{i + 1}/{num_batches}], LR: {lr}, time left: {time_left} h")
        ...
        # save model
        path = "./saving/eveframe123_epoch_" + str(epoch) +f"_batch_{num_batches}"+"_time_"+utc_time.strftime('%Y%m%d_%H%M%S')+".pth"
        torch.save(model.state_dict(), path)
        artifact = wandb.Artifact('model', type='model')
        artifact.add_file(path)
        run.log_artifact(artifact)

    model_parameters_final_path = "./saving/eveframe123_epoch_" + str(num_epochs-1) +f"_batch_{num_batches}"+"_time_"+utc_time.strftime('%Y%m%d_%H%M%S')+".pth"
