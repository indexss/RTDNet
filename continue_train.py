...
if __name__ == '__main__':
    ...
    model.load_state_dict(torch.load('eveframe_epoch_6_batch_4069_time_20240802_080050.pth'))
    # model = FaceMixSTE().to(device)
    model.train()

    num_epochs = 3
    total_epochs = 10
# 先训练了n=total_epochs-num_epochs个epoch，然后再训练num_epochs个epoch
# 设置学习率，试图让学习率平滑一点
    n=total_epochs-num_epochs
    learning_rate = 0.0005 * (0.5**(n-1))
    ...
    for epoch in range(total_epochs-num_epochs, total_epochs):
        model.train()
        ...
        for i, data in enumerate(dataloader):
            ...
            for k, v in data.items():
                if isinstance(v, torch.Tensor):
                    data[k] = v.to(device)
            if hp['do_mask']:
                data['face_patch'] = mask_tensor(data['face_patch'], make_mask_indices(sequence_length=30,task='hard'), mask_type='noise')
            ...
            print(f"Epoch [{epoch + 1}/{total_epochs}], Batch [{i + 1}/{num_batches}], LR: {lr}")
        lr_now = optimizers.state_dict()["param_groups"][0]["lr"]
        lr_next = lr_now * 0.5
        for param_group in optimizers.param_groups:
            param_group['lr'] = lr_next
