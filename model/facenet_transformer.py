...
class FaceNetTransformer(nn.Module):
    def __init__(self):
        super(FaceNetTransformer, self).__init__()
    ...
    def forward(self, full_input_dict):
        ...
        full_intermediate_dict['face_g_tobii'] = full_input_dict['face_g_tobii']
        full_intermediate_dict['face_PoG_tobii'] = full_input_dict['face_PoG_tobii']
        full_intermediate_dict['face_PoG_cn_tobii'] = full_input_dict['face_PoG_cm_tobii']
        full_intermediate_dict['face_g_tobii_validity'] = full_input_dict['face_g_tobii_validity']
        ...
        for side in ['face']:
            ...
            gt_angulars_mean = t0 Max-Q / Mobile [Discrete]
                                         GPU 2: AMD Phoenix3 [Integrat'_PoG_cm_initial'
...
class MyResNet18(nn.Module):
    def __init__(self):
        ...
        self.downsample1 = nn.Sequential(
            nn.Conv2d(5, 1, 1), # 29 x 64
            nn.InstanceNorm2d(1)
        )


        self.downsample2 = nn.Sequential(
            nn.Conv2d(5, 1, 1), # 29 * 128
            nn.InstanceNorm2d(1)
        )

        self.downsample2_1 = nn.Sequential(
            nn.Conv2d(128, 64, 1), # 29 x 128
            nn.InstanceNorm2d(64)
        )

        self.downsample3 = nn.Sequential(
            nn.Conv2d(5, 1, 1), # 29 x 256
            nn.InstanceNorm2d(1)
        )

        self.downsample3_1 = nn.Sequential(
            nn.Conv2d(256, 128, 1), # 29 x 256
        ...
        self.downsample4_1 = nn.Sequential(
            nn.Conv2d(512, 256, 1), # 29 x 256
            nn.InstanceNorm2d(256)
        )

       

        self.fusionblock1 = BasicBlock(5, 1, downsample=self.downsample1)
        
        self.fusionblock2 = BasicBlock(5, 1, downsample=self.downsample2)
        self.fusionblock2_1 = BasicBlock(128, 64, downsample=self.downsample2_1)
        
        self.fusionblock3 = BasicBlock(5, 1, downsample=self.downsample3)
        self.fusionblock3_1 = BasicBlock(256, 128, downsample=self.downsample3_1)
        
        self.fusionblock4 = BasicBlock(5, 1, downsample=self.downsample4)
        self.fusionblock4_1 = BasicBlock(512, 256, downsample=self.downsample4_1)
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        # 前面补16 x 2 x 64 x 64 x 64的0，后面补16 x 3 x 64 x 64 x 64的0
        difference1 = torch.cat([torch.zeros((batch_size, 2, feature1_c, feature1_h, feature1_w)).to(difference1.device), difference1], dim=1)
        difference1 = torch.cat([difference1, torch.zeros((batch_size, 3, feature1_c, feature1_h, feature1_w)).to(difference1.device)], dim=1)
        # 16 x 34 x 64 x 64 x 64 -> 30 x (16 x 5 x 64 x 64 x 64)
        difference1_windows = sliding_window(difference1, window_size=5, step_size = 1)
        diff1_out = []
        for diff1 in difference1_windows:
            # diff1: 16 x 5 x 64 x 64 x 64 -> 16 x 64 x 5 x 64 x 64
            diff1 = diff1.permute(0, 2, 1, 3, 4)
            # diff1: 16 x 64 x 5 x 64 x 64 -> 16 x 64 x 1 x 64 x 64
            diff1 = diff1.reshape(batch_size*feature1_c, 5, feature1_h, feature1_w)
            diff1 = self.fusionblock1(diff1)
            diff1 = diff1.reshape(batch_size, feature1_c, 1, feature1_h, feature1_w)
            # diff1: 16 x 64 x 1 x 64 x 64 -> 16 x 1 x 64 x 64 x 64
            diff1 = diff1.permute(0, 2, 1, 3, 4)
            diff1_out.append(diff1)
        # differrence1_bind: 16 x 30 x 64 x 64 x 64
        difference1_bind = torch.cat(diff1_out, dim=1)
        # feature1: 16 x 30 x 64 x 64 x 64
...
# -------------------------------------------------------------------------------------
        ...
        # 前面补16 x 2 x 128 x 32 x 32的0，后面补16 x 3 x 128 x 32 x 32的0
        difference2 = torch.cat([torch.zeros((batch_size, 2, feature2_c, feature2_h, feature2_w)).to(difference2.device), difference2], dim=1)
        difference2 = torch.cat([difference2, torch.zeros((batch_size, 3, feature2_c, feature2_h, feature2_w)).to(difference2.device)], dim=1)
        # 16 x 34 x 128 x 32 x 32 -> 30 x (16 x 5 x 128 x 32 x 32)
        difference2_windows = sliding_window(difference2, window_size=5, step_size = 1)
        diff2_out = []
        for diff2 in difference2_windows:
            # diff2: 16 x 5 x 128 x 32 x 32 -> 16 x 128 x 5 x 32 x 32
            diff2 = diff2.permute(0, 2, 1, 3, 4)
            # diff2: 16 x 128 x 5 x 32 x 32 -> 16 x 128 x 1 x 32 x 32
            diff2 = diff2.reshape(batch_size*feature2_c, 5, feature2_h, feature2_w)
            diff2 = self.fusionblock2(diff2)
            diff2 = diff2.reshape(batch_size, feature2_c, 1, feature2_h, feature2_w)
            # diff2: 16 x 128 x 1 x 32 x 32 -> 16 x 1 x 128 x 32 x 32
            diff2 = diff2.permute(0, 2, 1, 3, 4)
            diff2_out.append(diff2)
        
        # differrence2_bind: 16 x 30 x 128 x 32 x 32
        difference2_bind = torch.cat(diff2_out, dim=1)
        # feature2: 16 x 30 x 128 x 32 x 32
...
# -------------------------------------------------------------------------------------
        ...
        # 前面补16 x 2 x 256 x 16 x 16的0，后面补16 x 3 x 256 x 16 x 16的0
        difference3 = torch.cat([torch.zeros((batch_size, 2, feature3_c, feature3_h, feature3_w)).to(difference3.device), difference3], dim=1)
        difference3 = torch.cat([difference3, torch.zeros((batch_size, 3, feature3_c, feature3_h, feature3_w)).to(difference3.device)], dim=1)
        # 16 x 34 x 256 x 16 x 16 -> 30 x (16 x 5 x 256 x 16 x 16)
        difference3_windows = sliding_window(difference3, window_size=5, step_size = 1)
        diff3_out = []
        for diff3 in difference3_windows:
            # diff3: 16 x 5 x 256 x 16 x 16 -> 16 x 256 x 5 x 16 x 16
            diff3 = diff3.permute(0, 2, 1, 3, 4)
            # diff3: 16 x 256 x 5 x 16 x 16 -> 16 x 256 x 1 x 16 x 16
            diff3 = diff3.reshape(batch_size*feature3_c, 5, feature3_h, feature3_w)
            diff3 = self.fusionblock3(diff3)
            diff3 = diff3.reshape(batch_size, feature3_c, 1, feature3_h, feature3_w)
            # diff3: 16 x 256 x 1 x 16 x 16 -> 16 x 1 x 256 x 16 x 16
            diff3 = diff3.permute(0, 2, 1, 3, 4)
            diff3_out.append(diff3)
        # differrence3_bind: 16 x 30 x 256 x 16 x 16
...
# -------------------------------------------------------------------------------------    
        ...
        # 前面补16 x 2 x 512 x 8 x 8的0，后面补16 x 3 x 512 x 8 x 8的0
        difference4 = torch.cat([torch.zeros((batch_size, 2, feature4_c, feature4_h, feature4_w)).to(difference4.device), difference4], dim=1)
        difference4 = torch.cat([difference4, torch.zeros((batch_size, 3, feature4_c, feature4_h, feature4_w)).to(difference4.device)], dim=1)
        # 16 x 34 x 512 x 8 x 8 -> 30 x (16 x 5 x 512 x 8 x 8)
        difference4_windows = sliding_window(difference4, window_size=5, step_size = 1)
        diff4_out = []
        for diff4 in difference4_windows:
            # diff4: 16 x 5 x 512 x 8 x 8 -> 16 x 512 x 5 x 8 x 8
            diff4 = diff4.permute(0, 2, 1, 3, 4)
            # diff4: 16 x 512 x 5 x 8 x 8 -> 16 x 512 x 1 x 8 x 8
            diff4 = diff4.reshape(batch_size*feature4_c, 5, feature4_h, feature4_w)
            diff4 = self.fusionblock4(diff4)
            diff4 = diff4.reshape(batch_size, feature4_c, 1, feature4_h, feature4_w)
            # diff4: 16 x 512 x 1 x 8 x 8 -> 16 x 1 x 512 x 8 x 8
            diff4 = diff4.permute(0, 2, 1, 3, 4)
            diff4_out.append(diff4)
        # differrence4_bind: 16 x 30 x 512 x 8 x 8
        difference4_bind = torch.cat(diff4_out, dim=1)

# -------------------------------------------------------------------------------------
        # d4_bind_up: (16 x 30) x 512 x 8 x 8 -> (16 x 30) x 512 x 16 x 16
        difference4_bind = difference4_bind.reshape((batch_size*seq_len, feature4_c, feature4_h, feature4_w))
        difference4_bind_up = F.interpolate(difference4_bind, scale_factor=2, mode='nearest')
        # d4_bind_up: (16 x 30) x 512 x 16 x 16 -> (16 x 30) x 256 x 16 x 16
        # print("-----------------------------------------")
        # print(difference4_bind_up.shape)
        difference4_bind_up = self.fusionblock4_1(difference4_bind_up)
        difference4_bind_up = difference4_bind_up.reshape((batch_size, seq_len, feature3_c, feature3_h, feature3_w))
        difference3_bind = difference4_bind_up + difference3_bind

        # d3_bind_up: 16 x 30 x 256 x 16 x 16 -> 16 x 30 x 256 x 32 x 32
        difference3_bind = difference3_bind.reshape((batch_size*seq_len, feature3_c, feature3_h, feature3_w))
        difference3_bind_up = F.interpolate(difference3_bind, scale_factor=2, mode='nearest')
        # d3_bind_up: (16 x 30) x 256 x 32x 32 -> (16 x 30) x 128 x 32 x 32
        difference3_bind_up = self.fusionblock3_1(difference3_bind_up)
        difference3_bind_up = difference3_bind_up.reshape((batch_size, seq_len, feature2_c, feature2_h, feature2_w))
        difference2_bind = difference3_bind_up + difference2_bind

        # d2_bind_up: 16 x 30 x 128 x 32 x 32 -> 16 x 30 x 128 x 64 x 64
        difference2_bind = difference2_bind.reshape((batch_size*seq_len, feature2_c, feature2_h, feature2_w))
        difference2_bind_up = F.interpolate(difference2_bind, scale_factor=2, mode='nearest')
        # d2_bind_up: (16 x 30) x 128 x 64 x 64 -> (16 x 30) x 64 x 64 x 64
        difference2_bind_up = self.fusionblock2_1(difference2_bind_up)
        difference2_bind_up = difference2_bind_up.reshape((batch_size, seq_len, feature1_c, feature1_h, feature1_w))
        difference1_bind = difference2_bind_up + difference1_bind
       
        difference1_bind = difference1_bind.reshape((batch_size*seq_len), feature1_c, feature1_h, feature1_w)
        feature1 = feature1 + difference1_bind
        feature2 = self.layer2(feature1)
        difference2_bind = difference2_bind.reshape((batch_size*seq_len), feature2_c, feature2_h, feature2_w)
        feature2 = feature2 + difference2_bind
